...
$user = current_user(); $query = db()->prepare('SELECT s.id, s.domicile, s.education_preference, a.id AS academic_id, aa.id AS academic_achievement_id, na.id AS non_academic_achievement_id, o.id AS organization_id, (SELECT COUNT(*) FROM interest_scores i WHERE i.student_id=s.id) AS interest_count FROM students s LEFT JOIN academic_scores a ON a.student_id=s.id LEFT JOIN academic_achievements aa ON aa.student_id=s.id LEFT JOIN non_academic_achievements na ON na.student_id=s.id LEFT JOIN organization_scores o ON o.student_id=s.id WHERE s.user_id=?'); $query->execute([$user['id']]); $state = $query->fetch() ?: [];
